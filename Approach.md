...
* **Deterministic Feature Mapping:** Instead of relying on non-deterministic LLM logic to classify test categories, the application processes the database programmatically on boot: items containing `"Knowledge & Skills"` inside their keys are explicitly mapped to `test_type = "K"`, while items featuring `"Personality & Behavior"` or `"Competencies"` are mapped to `test_type = "P"`. This ensures 100% precision for the categorization requirement.
